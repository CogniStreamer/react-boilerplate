...
You can remove `react-intl` modules by first removing the `IntlProvider` object from the `app/app.jsx` file and by either removing or not selecting the i18n text option during component scaffolding.
